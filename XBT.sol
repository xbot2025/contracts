...
// File: contracts/ITrade.sol

// SPDX-License-Identifier: MIT
pragma solidity ^0.8.18;

interface ITrade {
  function getTradeSmartVault() external view returns (address);
...
contract XBT is IBEP20, Ownable {
  using SafeMath for uint256;
  ...
  constructor(address router, address usdt, address receiver) {
    ...
    // Initialize usdt fee smart vault
    _feeSmartVault = new FeeSmartVault(usdt);
  ...
  function _transfer(
    ...
    if ((_v2Pairs[from] || _v2Pairs[to]) && from != address(this)) {
      ...
      address _tradeSmartVault = _trade.getTradeSmartVault();

      // Can only purchase through Dapp within the limit time, free trading after that.
      ...
      if (
        // Only on sell
        isSell &&
        // Adding liquidity is not a sell
